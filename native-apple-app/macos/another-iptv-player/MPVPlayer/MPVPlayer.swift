...
extension MPVPlayer {
  ...
  /// External SRT/SUB dosyasını mpv'ye yükler ve seçili hale getirir. `sub-add ... select`
  /// flag'i ile yükleme + aktivasyon tek komutta yapılır; başarı/başarısızlık event log'a yazılır.
  /// File:// URL string olarak verilmeli; sandbox altında security-scoped resource access
  /// caller'ın sorumluluğunda (NSOpenPanel zaten verir).
  func loadExternalSubtitle(url: URL, title: String? = nil, lang: String? = nil) {
    mpvQueue.async { [weak self] in
      guard let self, let handle = self.mpv, !self.isDisposed else { return }
      var args: [String] = ["sub-add", url.path, "select"]
      if let title, !title.isEmpty { args.append(title) }
      if let lang, !lang.isEmpty { args.append(lang) }
      var cArgs = args.map { strdup($0) }
      defer { cArgs.forEach { free($0) } }
      var argv: [UnsafePointer<CChar>?] = cArgs.map { UnsafePointer($0) }
      argv.append(nil)
      ...
      if st < 0 {
        MPVPlayerVideoLog.always(
          "Player.subAdd",
          "mpv_command failed: \(String(cString: mpv_error_string(st))) (code \(st))"
        )
      }
    }
  }
