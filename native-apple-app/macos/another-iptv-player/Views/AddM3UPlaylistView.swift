...
struct AddM3UPlaylistView: View {
    ...
    /// Drag & drop / Open File ile gelen URL — view açılır açılmaz dosyayı işler.
    private let prefilledFileURL: URL?

    init(editingPlaylist: Playlist? = nil, prefilledFileURL: URL? = nil) {
        self.editingPlaylist = editingPlaylist
        self.prefilledFileURL = prefilledFileURL
        _name = State(initialValue: editingPlaylist?.name ?? prefilledFileURL?.deletingPathExtension().lastPathComponent ?? "")
    ...
    var body: some View {
        NavigationStack {
            ...
            .task {
                // Drag & drop / Open File ile gelen URL'i view açılınca otomatik yükle.
                if let url = prefilledFileURL, !hasLocalFile {
