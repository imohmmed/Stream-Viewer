...
struct M3UGroupShelfRow: View, Equatable {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if items.isEmpty {
                ...
                PagedHorizontalShelf(
                    ...
                    onPageChange: { range in
                        // Sayfa atlanınca yeni görünür aralığın bir önündeki son kanaldan başlayarak prefetch tetikle.
                        prefetchAhead(from: range.lowerBound - 1, count: range.count + M3UShelf.initialWarmup)
                    }
                ) { channel in
                    M3UChannelCard(
                        channel: channel,
                ...
                .onAppear {
                    // Sadece ilk pencere ısıtması; sayfa atlanınca onPageChange tetiklenir.
    ...
    private var groupHeader: some View {
        ...
        if let selector = sidebarCategorySelector {
            Button {
                selector.select(.channels, group)
            } label: {
                label
            }
            .buttonStyle(.plain)
